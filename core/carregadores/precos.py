...
class CarregadorPrecos:
    ...
    def arquivo_existe(self) -> bool:
        ...
        return os.path.exists(self.config.arquivo_precificacao)
    ...
    def carregar(self, forcar_recarga: bool = False) -> bool:
        ...
        if not self.arquivo_existe():
            self._erros.append(f"Arquivo não encontrado: {self.config.arquivo_precificacao}")
            raise FileNotFoundError(
                f"Base de Precificação não encontrada: {self.config.arquivo_precificacao}"
            )
        
        try:
            # Abre a planilha
            workbook = openpyxl.load_workbook(
                self.config.arquivo_precificacao, 
                data_only=True
            )
    ...
    def _mapear_colunas_contas(self, mapa_colunas: Dict[str, int]) -> Dict[str, int]:
        ...
        for prefixo, nome_conta in self.config.mapa_prefixo_conta.items():
    ...
    def obter_preco(self, sku_completo: str) -> Optional[str]:
        ...
        for prefixo in self.config.mapa_prefixo_conta.keys():
